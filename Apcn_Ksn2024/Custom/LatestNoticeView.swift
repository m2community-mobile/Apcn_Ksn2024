...
class LatestNoticeView: UIView {
    ...
    override init(frame: CGRect) {
        ...
        backView.backgroundColor = #colorLiteral(red: 0.2213132083, green: 0.2135579884, blue: 0.3757383227, alpha: 1)
...
//        noticeLabel = UILabel(frame: CGRect(x: iconImageBackView.frame.maxX, y: 0, width: self.frame.size.width - iconImageBackView.frame.maxX, height: self.frame.size.height))
        noticeLabel = UILabel(frame: CGRect(x: iconImageBackView.frame.maxX, y: 0, width: 55, height: self.frame.size.height))
        noticeLabel.font = UIFont(name: ROBOTO_MEDIUM, size: noticeLabel.frame.size.height * 0.33)
        noticeLabel.textColor = UIColor.white
        noticeLabel.text = "Notice"
        self.addSubview(noticeLabel)
        
        
        valueLabel = UILabel(frame: CGRect(x: noticeLabel.frame.maxX, y: 0, width: self.frame.size.width - iconImageBackView.frame.maxX, height: self.frame.size.height))
        valueLabel.font = UIFont(name: ROBOTO_MEDIUM, size: valueLabel.frame.size.height * 0.33)
        valueLabel.textColor = UIColor.white
        valueLabel.text = "Notice"
