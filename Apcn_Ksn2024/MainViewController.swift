...
//        self.view.addSubview(boothEventButton)
        ...
        if IS_IPHONE_12PRO_MAX {
            mainButtonBackView = UIView(frame: CGRect(x: 0, y: bottomView.frame.minY - mainButtonBackViewHeight - 30, width: SCREEN.WIDTH, height: mainButtonBackViewHeight + 30))
        }
        if IS_IPHONE_15PRO_MAX {
            mainButtonBackView = UIView(frame: CGRect(x: 0, y: bottomView.frame.minY - mainButtonBackViewHeight - 30, width: SCREEN.WIDTH, height: mainButtonBackViewHeight + 30))
        }
        if IS_IPHONE_12PRO {
            mainButtonBackView = UIView(frame: CGRect(x: 0, y: bottomView.frame.minY - mainButtonBackViewHeight - 20, width: SCREEN.WIDTH, height: mainButtonBackViewHeight + 20))
        }
        if IS_IPHONE_15PRO {
            mainButtonBackView = UIView(frame: CGRect(x: 0, y: bottomView.frame.minY - mainButtonBackViewHeight - 20, width: SCREEN.WIDTH, height: mainButtonBackViewHeight + 20))
        }
...
//        }
        ...
        if IS_IPHONE_12PRO {
            noticeView = LatestNoticeView(frame: CGRect(x: 0, y: mainButtonBackView.frame.maxY - 41, width: SCREEN.WIDTH, height: noticeViewHeight - 9))
        }
        if IS_IPHONE_15PRO {
            noticeView = LatestNoticeView(frame: CGRect(x: 0, y: mainButtonBackView.frame.maxY - 41, width: SCREEN.WIDTH, height: noticeViewHeight - 9))
        }
        ...
        if IS_IPHONE_N {
            noticeView = LatestNoticeView(frame: CGRect(x: 0, y: mainButtonBackView.frame.maxY - 40, width: SCREEN.WIDTH, height: noticeViewHeight - 10))
        }
        if IS_IPHONE_MINI {
            noticeView = LatestNoticeView(frame: CGRect(x: 0, y: mainButtonBackView.frame.maxY - 40, width: SCREEN.WIDTH, height: noticeViewHeight - 10))
        }
...
//            }
            let button = MainButton(frame: CGRect(
                ...
                y: buttonY,
                width: buttonWidth - 4,
                height: buttonHeight - 4), imageName: "icon0\(i+1)", name: titleString)
            button.addTarget(event: .touchUpInside) { (button) in
                let infoDic = INFO.MAIN_INFO[i]
                contentShow(dataDic: infoDic as [String:Any])
            }
...
//        mainButtonBackView2.backgroundColor = UIColor.red
        ...
        if IS_IPHONE_12PRO_MAX {
            topTitle = UIImageView(frame: CGRect(x: 0, y: naviBar.frame.maxY, width: SCREEN.WIDTH * 0.8, height: 60))
        }
        if IS_IPHONE_15PRO_MAX {
            topTitle = UIImageView(frame: CGRect(x: 0, y: naviBar.frame.maxY, width: SCREEN.WIDTH * 0.8, height: 60))
        }
        ...
        if IS_IPHONE_12PRO {
            topTitle = UIImageView(frame: CGRect(x: 0, y: naviBar.frame.maxY, width: SCREEN.WIDTH * 0.8, height: 54))
        }
        if IS_IPHONE_15PRO {
            topTitle = UIImageView(frame: CGRect(x: 0, y: naviBar.frame.maxY, width: SCREEN.WIDTH * 0.8, height: 54))
        }
        ...
        if IS_IPHONE_N {
            topTitle = UIImageView(frame: CGRect(x: 0, y: naviBar.frame.maxY, width: SCREEN.WIDTH * 0.8, height: 45))
        }
        if IS_IPHONE_N_PLUS {
            topTitle = UIImageView(frame: CGRect(x: 0, y: naviBar.frame.maxY, width: SCREEN.WIDTH * 0.8, height: 55))
        }
        if IS_IPHONE_MINI {
            topTitle = UIImageView(frame: CGRect(x: 0, y: naviBar.frame.maxY, width: SCREEN.WIDTH * 0.8, height: 50))
        }
...
//        let leftBottomPoint2 = CGPoint(x: 0, y: self.height - smallRadius)
        ...
        if IS_IPHONE_MINI {
            nameLabel.font = UIFont(name: Pretendard_SemiBold, size: nameLabel.frame.size.height * 0.27)
        }
        if IS_IPHONE_N {
            nameLabel.font = UIFont(name: Pretendard_SemiBold, size: nameLabel.frame.size.height * 0.24)
        }
