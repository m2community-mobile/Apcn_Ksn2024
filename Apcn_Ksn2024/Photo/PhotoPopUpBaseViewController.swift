...
class PhotoPopUpBaseViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        navigationController?.navigationBar.barTintColor = mainColor2
        ...
        addTitle = UILabel(frame: CGRect(x: 0, y: likeButton.frame.minY - 20, width: SCREEN.WIDTH , height: buttonBackView.frame.size.height / 2))
        addTitle.backgroundColor = .white
        buttonBackView.addSubview(addTitle)
        
        
        nameTile = UILabel(frame: CGRect(x: 0, y: likeButton.frame.minY - 20, width: 70, height: buttonBackView.frame.size.height / 2))
//        nameTile.text = "테스트  |"
//        nameTile.layer.borderWidth = 0.3
//        nameTile.sizeToFit()
...
//        buttonBackView.addSubview(nameTile)
        
        countryTitle = UILabel(frame: CGRect(x: nameTile.frame.maxX, y: likeButton.frame.minY - 20, width: 70, height: buttonBackView.frame.size.height / 2))
        countryTitle.text = ""
//        countryTitle.sizeToFit()
//        countryTitle.layer.borderWidth = 0.3
        
//        buttonBackView.addSubview(countryTitle)
        
        dateTitle = UILabel(frame: CGRect(x: countryTitle.frame.maxX, y: likeButton.frame.minY - 20, width: 110, height: buttonBackView.frame.size.height / 2))
        dateTitle.text = "2024.05.18"
//        dateTitle.layer.borderWidth = 0.3
        
//        buttonBackView.addSubview(dateTitle)
...
//        let closeButtonItem = UIBarButtonItem(title: "Close", style: UIBarButtonItem.Style.plain, target: self, action: #selector(close))
        let closeButtonItem = UIBarButtonItem(image: UIImage(named: "btnX2")?.withRenderingMode(.alwaysTemplate), style: UIBarButtonItem.Style.done, target: self, action: #selector(close))
        closeButtonItem.tintColor = UIColor.black
...
//        var dataDic = self.photoInfos[self.contentIndex]
    ...
    func buttonsUpdate(index : Int){
        ...
        var getTitle = dataDic["title"] as? String ?? ""
        
//        self.nameTile.text = dataDic["title"] as? String ?? ""
        
      
//        let sampleStr = getTitle
//        let newStr = sampleStr.replacingOccurrences(of: "2", with: "*")

        
        self.nameTile.text = "\(getTitle) |"
        print("nameTile.text\(getTitle)")
        if getTitle.count >= 3 {
            
            let originalString = getTitle
            let range = NSRange(location: 2, length: 0)
            let nsString = originalString as NSString
            var newString = nsString.replacingCharacters(in: range, with: "**_*")
            
            
            

//            if let dotRange = newString.range(of: "") {
                if let dotRange = newString.range(of: "_*") {
                newString.removeSubrange(dotRange.lowerBound..<newString.endIndex)
            }
            
            
            
            self.nameTile.text = "\(newString) |"
        }
        
        
        var getCountry = dataDic["etc1"] as? String ?? ""
        
//        self.nameTile.text = dataDic["title"] as? String ?? ""
        
        self.countryTitle.text = "\(getCountry)  |"
        
        
        var getDate = dataDic["date"] as? String ?? ""
        
//        self.nameTile.text = dataDic["title"] as? String ?? ""
        
        self.dateTitle.text = "\(getDate)"
        
        

        if getTitle == "" {
            addTitle.text = "APCN & KSN 2024  |  \(dateTitle.text ?? "")"
        } else {
            
            addTitle.text = "\(nameTile.text ?? "")  \(countryTitle.text ?? "")  \(dateTitle.text ?? "")"
        }
